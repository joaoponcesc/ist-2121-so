...
int tfs_init() {
    ...
    if(pthread_mutex_init(&noof_mutex, NULL) != 0)
        return -1;
...
int tfs_destroy() {
    ...
    if(pthread_mutex_destroy(&noof_mutex) != 0)
        return -1;
    return 0;
}
...
int tfs_destroy_after_all_closed() {
    ...
    if(pthread_mutex_lock(&noof_mutex) != 0)
        return -1;
    while(number_of_open_files != 0){
        pthread_cond_wait(&cond, &noof_mutex);
    }
    if(pthread_mutex_unlock(&noof_mutex) != 0)
        return -1;
